...
public class SwipeActivity extends Fragment implements OnClickListener,
	...
	public void onItemClick(AdapterView<?> adp, View listview, int position,
		...
		if (adp != null && adp.getAdapter() instanceof SwipeAdapter) {
			...
			Toast.makeText(SwipeActivity.this.getActivity(),
	...
	private void init() {
		...
		DB = new DBAdapter(SwipeActivity.this.getActivity(), "D");
		...
		if (!flag) {
			String[] table_fileds = { "id", "text", "create_time", "tags",
					"gid", "synced", "time" };
	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.btn_love) {
			...
			// load layout
			LayoutInflater inflater = (LayoutInflater) SwipeActivity.this
			...
			Cursor cursor = DB.getReadableDatabase().query(true, "Entry",
					new String[] { "tags" }, null, null, null, null, null,
					null, null);
			cursor.moveToFirst();
			...
			final AutoCompleteTextView actv = (AutoCompleteTextView) layout
			...
			String tag = lstDiarys.get(pos).getTags();
			actv.setText(tag);

			// pop dialog
			new AlertDialog.Builder(SwipeActivity.this.getActivity(),
					0x00000003)
					.setTitle("Edit Item")
							...
							new DialogInterface.OnClickListener() {
								public void onClick(
									...
									if (str == null || str.equals("")) {
										...
										// modify to list
										final String tags = actv.getText()
												.toString();
										String id = lstDiarys.get(pos).getId();
										ContentValues cv = new ContentValues();
										cv.put("text", str);
										cv.put("tags", tags);
										DB.update("Entry", "rowId=?",
												new String[] { id }, cv);
					...
					.setNegativeButton("Cancel",
							new DialogInterface.OnClickListener() {
								public void onClick(
										DialogInterface dialoginterface, int i) {
									Toast.makeText(
											SwipeActivity.this.getActivity(),
											"Cancel add Item",
											Toast.LENGTH_SHORT).show();
		...
		if (v.getId() == R.id.btn_reload) {
			...
			int currentIndex = pos - listView.getFirstVisiblePosition()
					- listView.getHeaderViewsCount();
	...
	public void syncData2UI(int period) {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				categoryKey =  sharedPreferences.getString("category", "");
				filterKey =  sharedPreferences.getString("key", "");
				
				String  changeAC =  sharedPreferences.getString("change", "");
				
				if(changeAC.equals("true")){
					changeLocal = true;
				...
				if (!categoryKey.equals("") && !categoryKey.equals("ALL")) {
					...
					cursor = gdriveModel.DB.getReadableDatabase().query(
							...
							"rowID  desc");
					System.out.println("执行了 这一部分~！！！！！！！！！！！！！！！！！！！！！！all");
				...
				if (SwipeActivity.this != null) {
					if (SwipeActivity.this.getActivity() != null) {
						SwipeActivity.this.getActivity().runOnUiThread(
								new Runnable() {
									public void run() {
										if (changeLocal == true) {
											((SwipeAdapter) listView
													.getAdapter())
													.notifyDataSetChanged();
											changeLocal = false;
										}
										if (gdriveModel.changed
												&& gdriveModel.count == 0) {
											((SwipeAdapter) listView
													.getAdapter())
													.notifyDataSetChanged();
		...
		timer.schedule(task, 1000, per);// execute per 5s
	}
